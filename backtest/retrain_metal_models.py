...
def initialize_mt5(config):
    ...
    login_result = mt5.login(  # type: ignore
...
def get_historical_data(symbol, timeframe, start_date, end_date):
    ...
    # Get data from MT5
    rates = mt5.copy_rates_range(symbol, mt5_timeframe, start_date, end_date)  # type: ignore

    if rates is None or len(rates) == 0:
        logger.error(f"Failed to get data for {symbol}: {mt5.last_error()}")  # type: ignore
        return None
...
def prepare_training_data(df):
    ...
    # Moving averages
    df['sma_20'] = df['close'].rolling(window=20).mean()  # type: ignore
    df['sma_50'] = df['close'].rolling(window=50).mean()  # type: ignore
    df['ema_12'] = df['close'].ewm(span=12).mean()  # type: ignore
    df['ema_26'] = df['close'].ewm(span=26).mean()  # type: ignore
    ...
    # Bollinger Bands
    df['bb_middle'] = df['close'].rolling(window=20).mean()  # type: ignore
    bb_std = df['close'].rolling(window=20).std()  # type: ignore
    ...
    df['atr'] = ranges.max(axis=1).rolling(window=14).mean()  # type: ignore
    
    # Volatility
    df['volatility'] = df['returns'].rolling(window=20).std()  # type: ignore
    ...
    for symbol in symbols:
        ...
        for timeframe in timeframes:
            try:
                ...
                ml_predictor.train_symbol_model(symbol, timeframe, X, y)  # type: ignore
