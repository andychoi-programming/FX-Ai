...
print("[SEARCH] MT5 CONNECTION DIAGNOSTIC")
print("=" * 50)
...
if terminal_info is None:
    print("   [FAIL] Cannot get terminal info")
    print(f"   Error: {mt5.last_error()}")
else:
    print(f"   [PASS] Terminal connected: {terminal_info.connected}")
...
if account_info is None:
    ...
    print(f"   [PASS] Account balance: ${account_info.balance:.2f}")
...
for symbol in symbols_to_test:
    ...
    if symbol_info is None:
        print(f"   [FAIL] {symbol}: No symbol info")
    else:
        print(f"   [PASS] {symbol}: Available, spread: {symbol_info.spread}")
...
for symbol in symbols_to_test[:1]:  # Test just one
    ...
    if tick is None:
        print(f"   [FAIL] {symbol}: Cannot get tick data")
        print(f"   Error: {mt5.last_error()}")
    else:
        tick_time = datetime.fromtimestamp(tick.time)
        print(f"   [PASS] {symbol}: Tick received at {tick_time}")
...
if server_time:
    ...
    if time_diff > 60:
        print("   [WARN]  Large time difference detected!")
else:
    print("   [FAIL] Cannot get server time")

print("\n7. Testing position synchronization...")
positions = mt5.positions_get()
if positions is None:
    print("   [FAIL] Cannot get positions")
    print(f"   Error: {mt5.last_error()}")
else:
    print(f"   [PASS] Positions retrieved: {len(positions)} open positions")
...
if history is None:
    print("   [FAIL] Cannot get order history")
    print(f"   Error: {mt5.last_error()}")
else:
    print(f"   [PASS] Order history accessible: {len(history)} orders today")
